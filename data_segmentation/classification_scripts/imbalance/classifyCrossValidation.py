...
class ClassifyCV:
    ...
    def __init__(self, foldslist, dataset, clf, clf_label, trainingSet, classe="lastCol", resamplingtechnique=None,
    ...
    def addmetric(self):
        ...
        plt.savefig('./results/' + self.clf_label + '_' + self.techniqueLabel + '_' + training_set_name + '_' + test_set_name + '_' + time_in_string + '_binary.png')
        plt.close()
        ...
        plt.xlabel('false positive rate')
        plt.ylabel('true positive rate')
        plt.savefig('./results/' + self.clf_label + '_' + self.techniqueLabel + '_' + training_set_name + '_' + test_set_name + '_' + time_in_string + '_prob.png')
        plt.close()
    ...
    def showresults(self):
        ...
        # statistic
        colunas =['DataSet', 'Classifier', 'AUCROC_label', 'AUCROC_prob', 'Accuracy', 'Precision', 'Recall', 'F1_score']
        ...
        self.statisticsList.append(pd.DataFrame(dict(DataSet=self.dataset,
                                                     ...
                                                     AUCROC_label=self.listaAUC, AUCROC_prob=self.listaAUCProb,
                                                     ...
                                                     F1_score=f1_score(self.teste_Y.ravel(), self.y_pred.ravel())), columns=colunas))
