...
if __name__ == '__main__':
    ...
    for index, row in info.iterrows():
        ...
        for trainSet, testSet in zip(trainSets, testSets):
            ...
            for clf, name in zip(clfs, names):
                for technique, t_name in zip(sTechniques, technique_names):
                    ...
                    if apply_samp is True:
                        sm = SMOTE(kind='regular', random_state=7013)
            ...
            all_results.to_csv('results/ensemble_regular_smote.csv', columns=df_final.columns.values, index=False)
