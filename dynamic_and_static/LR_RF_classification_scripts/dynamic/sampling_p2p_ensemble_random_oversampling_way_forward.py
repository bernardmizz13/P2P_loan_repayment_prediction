...
if __name__ == '__main__':
    ...
    for index, row in info.iterrows():
        ...
        sTechniques = [RandomOverSampler(random_state=1)]
        technique_names = ["RO"]


        def getParamsReSampling(reSamplingTechnique):

            if type(reSamplingTechnique) is RandomOverSampler:
        ...
        for trainSet, testSet in zip(trainSets, testSets):
            ...
            all_results.to_csv('results/ensemble_random_oversampling.csv', columns=df_final.columns.values, index=False)
