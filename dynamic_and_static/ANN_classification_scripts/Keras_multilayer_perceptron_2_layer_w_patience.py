...
for index, row in hyperparameters_and_results.iterrows():
    ...
    if row['run'] is True:
        ...
        # callback for epoch patience
        patience_callback = EarlyStopping(monitor='val_loss', min_delta=0, patience=20, verbose=1, mode='min', restore_best_weights=True)
        
        history  = mlp.fit(X_train, Y_train, validation_data=(X_val, Y_val), epochs=train_epochs, batch_size=train_batch_size,
                           callbacks=[evaluate_after_epoch(model_no = index, x_test=X_test, y_test=Y_test, x_val = X_val, y_val = Y_val, x_train=X_train, y_train=Y_train), patience_callback])
