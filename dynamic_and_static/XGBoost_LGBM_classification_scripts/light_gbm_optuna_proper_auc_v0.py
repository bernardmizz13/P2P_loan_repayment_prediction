...
__file__ = './logs/light_gbm_log_' + str(datestring)
...
# callback function to store the best lgbmoost
...
hyperparameters.boosting_type =  hyperparameters.boosting_type.apply(lambda x: x[1:-1].split(' '))
...
hyperparameters.num_leaves =  hyperparameters.num_leaves.apply(lambda x: x[1:-1].split(' '))
...
hyperparameters.min_child_samples =  hyperparameters.min_child_samples.apply(lambda x: x[1:-1].split(' '))
...
for index, row in hyperparameters.iterrows():
    ...
    if row['run'] is True:
        ...
        print("Captured the general settings for the Light GBM")
        logging.info("Captured the general settings for the Light GBM")
        ...
        def objective(trial):
            ...
            num_leaves = hyperparameters.num_leaves[index]
            ...
            min_child_samples = hyperparameters.min_child_samples[index]
            ...
            num_leaves = [int(i) for i in num_leaves]
            ...
            min_child_samples = [int(i) for i in min_child_samples]
            
            # parameters for the xg boost
            param = {
                "objective" : trial.suggest_categorical('objective', hyperparameters.objective[index]),
                "boosting_type" : trial.suggest_categorical('boosting_type', hyperparameters.boosting_type[index]),
                ...
                "min_child_weight" : trial.suggest_loguniform('min_child_weight', *minchildweight),
                ...
                "min_child_samples" : trial.suggest_int('min_child_samples', *maxdepth)
            }
            
            # initialize the XG Boost classifier with the passed parameters
            model = LGBMClassifier(**param, random_state = int(hyperparameters.sub_sampling_seed[index]))
        ...
        # optimize and store the best lgbm model using a callback
        ...
        x_axis = range(0, len(results['valid_0']['auc']))
        
        # plot auc
        fig, ax = plt.subplots()
        ax.plot(x_axis, results['valid_0']['auc'], label='Validation')
        ax.legend()
        plt.ylabel('AUC ROC')
        ...
        save_best_trial_metrics(str(best_model.get_params()), model_num, val_auc_roc_from_best_model, Y_val, val_label_preds, val_raw, test_auc_roc_from_best_model, Y_test, test_label_preds, raw_preds, my_auc_roc_val, my_auc_roc_test, sklearn_test_auc_roc, sklearn_val_auc_roc)
        ...
        boosting_type_string = hyperparameters.boosting_type[index]
        scale_string = hyperparameters.scale_pos_weight[index]
        learningrate_string = hyperparameters.learning_rate[index]
        num_leaves_string = hyperparameters.num_leaves[index]
        ...
        hyperparameters.loc[index, 'boosting_type'] = '[' + ' '.join([str(elem) for elem in boosting_type_string]) + ']'
        ...
        hyperparameters.loc[index, 'num_leaves'] = '[' + ' '.join([str(elem) for elem in num_leaves_string]) + ']'
        ...
        hyperparameters.loc[index, 'min_child_samples'] = '[' + ' '.join([str(elem) for elem in min_child_samples]) + ']'
